...
async def test_notify_escalation_prefers_name_passed_from_the_call(tenant, fake_pool, monkeypatch):
    ...
    await voice_stream._notify_escalation(tenant, fake_pool, "+32470000001", "Peeters", "klacht")

    lookup_mock.assert_not_called()
    send_mock.assert_awaited_once_with(tenant, "+32470000001", "Peeters", "klacht")
...
async def test_notify_escalation_falls_back_to_lookup_when_no_name_given(tenant, fake_pool, monkeypatch):
    ...
    await voice_stream._notify_escalation(tenant, fake_pool, "+32470000001", "", "klacht")

    lookup_mock.assert_awaited_once()
    send_mock.assert_awaited_once_with(tenant, "+32470000001", "Janssens", "klacht")
